# Hyprland: the compositor itself, plus the session-wide settings.
#
# Its keybindings, autostart, idle/lock behaviour and screenshot bindings are
# separate files in this directory - all writing to the same `desktop`
# aggregate, which is what merges them back together.
#
# Quickshell (bar, launcher, clipboard history) and mako (notifications) are
# part of the same aggregate, so importing `desktop` brings up the whole
# session; they no longer have to be switched on from here.
{
  flake.modules.homeManager.desktop = { config, lib, pkgs, ... }: {
